  ...
  const handleShare = async () => {
    try {
      const dealInfo = wine.dealScore >= 80
        ? ` (${wine.dealScore >= 95 ? '\u{1F3C6} TROUVAILLE' : wine.dealScore >= 88 ? '\u{1F525} Aubaine' : '\u{1F44D} Bon rapport Q/P'})`
        : '';
      const ratingStars = noteRating > 0 ? '\n' + '\u{2B50}'.repeat(noteRating) + ` (${noteRating}/5)` : '';
      const userNoteText = existingNote?.note ? `\n\u{1F4DD} ${existingNote.note}` : '';
      await Share.share({
        message: `\u{1F377} ${wine.name}\n\u{1F4B0} ${wine.price?.toFixed(2)}$${dealInfo}${ratingStars}${userNoteText}\n\u{1F517} ${wine.saqUrl}`,
      });
    } catch (_) {
...
const styles = StyleSheet.create({
  ...
  secondaryBtnActive: { borderColor: COLORS.gold + '60', backgroundColor: COLORS.gold + '10' },
  ...
  starsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: SPACING.sm },
  starsLabel: { fontSize: 14, color: COLORS.gray },
  stars: { flexDirection: 'row', gap: 4 },
  noteInput: {
    borderWidth: 1, borderColor: COLORS.grayLight, borderRadius: RADIUS.sm,
    padding: SPACING.sm, fontSize: 14, color: COLORS.black, minHeight: 80, textAlignVertical: 'top',
  },
  noteDisplay: {
    flexDirection: 'row', alignItems: 'flex-start', gap: SPACING.sm,
    padding: SPACING.sm, backgroundColor: COLORS.cream, borderRadius: RADIUS.sm,
  },
